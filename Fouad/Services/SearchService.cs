...
namespace Fouad.Services
{
    ...
    public class SearchService : ISearchService
    {
        ...
        public SearchService(IFileDataService fileDataService, IConfigurationService configurationService)
        {
        ...
        public async Task<List<Result>> AdvancedSearchAsync(SearchCriteria criteria)
        {
            return await RetryHelper.ExecuteWithRetryAsync(
                async () =>
                {
                    ...
                    if (!_fileDataService.IsFileLoaded() || string.IsNullOrEmpty(criteria.SearchTerm))
                    {
                        return results;
                    }
                    
                    // Get all results from file data service
                    var allResults = await Task.Run(() => GetAllResults());
                    
                    // Apply search criteria
                    var filteredResults = await Task.Run(() => ApplySearchCriteria(allResults, criteria));
                    
                    // Limit results
                    return filteredResults.Take(criteria.MaxResults).ToList();
                },
                maxRetries: 3,
        ...
        public List<string> GetSearchSuggestions(string partialInput, int maxSuggestions = 10)
        {
            try
            {
                ...
                // For now, we'll return an empty list as this would require a more complex implementation
                // In a real implementation, this would query the index for partial matches
                return new List<string>();
            }
            catch (Exception ex)
            {
        ...
        private List<Result> GetAllResults()
        {
            ...
            try
            {
                ...
                // In a real implementation, you would retrieve results more efficiently
                for (int i = 1; i <= Math.Min(rowCount, 1000); i++)
                {
        ...
        /// <summary>
        /// Applies search criteria to filter results.
        /// </summary>
        /// <param name="results">List of results to filter</param>
        /// <param name="criteria">Search criteria</param>
        /// <returns>Filtered list of results</returns>
        private List<Result> ApplySearchCriteria(List<Result> results, SearchCriteria criteria)
        {
            try
            {
                return results.Where(result => MatchesCriteria(result, criteria)).ToList();
            }
            catch (Exception ex)
            {
                // Log the exception
                LoggingService.LogError($"Error in ApplySearchCriteria: {ex.Message}", ex);
                throw new InvalidOperationException(string.Format(ErrorMessages.ErrorPerformingSearch, ex.Message), ex);
            }
        }
        ...
        private static bool RegexSearch(string text, string searchTerm, bool caseSensitive)
        {
            try
            {
                ...
                return System.Text.RegularExpressions.Regex.IsMatch(text, searchTerm, options);
            }
            catch (Exception ex)
            {
